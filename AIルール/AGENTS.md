...
1. 作業開始前にプロジェクトルートの `AGENTS.md`を読む。
2. 編集対象ファイルのディレクトリからルートに向かって各階層の `AGENTS.md` / ルールファイルを探索し、すべて読む。
...
5. **Test & Docs**  
   ...
   - コード変更を行った場合は同じセッション内で関連テストを実行し、結果（使用コマンド・環境・成否）を報告する。実行できない場合は理由と代替確認を明記する。  
   - テスト完了後、該当 Issue の `docs/plan.md` の AC チェックを更新してから完了報告する（Plan同期必須）。  
...
以上をデフォルトの行動方針とし、プロジェクト固有の `AGENTS.md` / ルールファイルがあれば、それらで上書き・補完する。
